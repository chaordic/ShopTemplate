...
function redirect() {
	if (top.spl.cid_redirect[top.spl.cid]) {
		if (/utm_/.test(top.location.search)) {
			var redirect = top.location.href.replace(/utm_source=.*?(&|$)/, "utm_source=ShopBack$1");
			if (top.location.href.indexOf('utm_medium') === -1) {
				redirect = redirect + "&utm_medium=ShopConvert_Leads"
			} else {
				redirect = redirect.replace(/utm_medium=.*?(&|$)/, "utm_medium=ShopConvert_Leads$1")
			}
		} else if (top.location.href.indexOf('?') === -1) {
			var redirect = top.location.href + '?utm_source=ShopBack&utm_medium=ShopConvert_Leads'
		} else {
			var redirect = top.location.href + '&utm_source=ShopBack&utm_medium=ShopConvert_Leads'
		}
		top.spl.set_cookie('_spl_tr', top.spl.cid, 720);
		top.location.href = redirect.split("#/cart").join("")
	}
}
...
function link_transform(url) {
	if (/utm_/.test(url)) {
		...
		var redirect = url + '?utm_source=ShopBack&utm_medium=ShopConvert_Leads'
	} else {
		var redirect = url + '&utm_source=ShopBack&utm_medium=ShopConvert_Leads'
	}
	return redirect
