...
SB.begin = function() {
    ...
    var begin = setInterval(function() {
        if (typeof $ != "undefined") {
            ...
            // console.log("\nSB.carrossel:");
            // console.log(SB.carrossel);


            var Class = SB.ov.attr("class"),
                vitrine = /(vitrine)+/,
                recomendacao = /(recomendacao|sb_overlay_social)+/;
...
SB.facebook = function(fId) {
    ...
    // REDIRECT 
    window.val_close_redirect == "redirect";
        ...
        FB.login(function(response) {
            if (response.authResponse && String(response.authResponse.grantedScopes).search("email") > -1) {
                ...
                FB.api('/me?fields=name,email,gender', function(user) {
                    ...
                    $(".user_name").html(String(user.name).split(" ")[0]);
...
function shopConvertAjax(user, response) {
    $.ajax({
        ...
        success: function(data) {
            ...
            try { top._st_app.ci(String(user.email));
                top._st_app.set_name(String(user.name)); } catch (e) {}

            // Marca tag de conversão
            top.spl.set_cookie('_spl_overlay_social', "disabled", 720);

            // TOGLE SOCIAL_CALL / SOCIAL_THANKS
            $(".social_call").fadeOut(function(){
                $(".social_thanks").fadeIn();
                $(".social_thanks").show();
            });
...
function clickURL(l) {
    if (l.search("utm_source=ShopBack") <= -1 && typeof top.spl.link_transform !== 'undefined') {
        ...
        if (typeof window.top.ShopBack == "object") {
            ...
            if (typeof window.top.ShopBack.PreventUtmsDefault != "undefined") l = (window.top.ShopBack.PreventUtmsDefault) ? l : top.spl.link_transform(l);
...
SB.getProducts = function(option) {
    top.shopback.getRecommendations(option, function(response, erro) {
